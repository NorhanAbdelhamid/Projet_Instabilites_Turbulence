###______________ Attracteur de Rössler ______________###
...
# 1] Définir les équations de Rössler : 
def rossler(variables, t):
    # Variables : 
    x, y, z = variables
...
# 3] Définir le temps : 
intervalle_temps = (0, 200)
temps = np.linspace(intervalle_temps[0], intervalle_temps[1], 10000)

# 4] Résoudre les équations de Lorenz : 
solution = solve_ivp(lambda t, y: rossler(y, t), intervalle_temps, variables_initiales, t_eval=temps, method='RK45')
x, y, z = solution.y
...
ax.set_title("Attracteur de Rössler")
ax.set_xlabel("X")
ax.set_ylabel("Y")
ax.set_zlabel("Z")
plt.savefig('Rossler_visuel')
plt.show()
...
condition_initiale_a = [1, 2, 3]
condition_initiale_b = [3, 5, 6]
...
# 3] Résoudre les équations de Rössler : 
solution_a = solve_ivp(lambda t, y: rossler(y, t), intervalle_temps_CI, condition_initiale_a, t_eval=temps_CI)
solution_b = solve_ivp(lambda t, y: rossler(y, t), intervalle_temps_CI, condition_initiale_b, t_eval=temps_CI)
...
plt.title('Histogramme de x (condition initiale [1, 2, 3])')
plt.xlabel('x')
plt.ylabel('Density')

plt.subplot(1, 2, 2)
plt.hist(valeurs_x_b, bins=60, density=True, color='red')
plt.title('Histogramme de x (condition initiale [3, 5, 6])')
plt.xlabel('x')
plt.ylabel('Density')

plt.tight_layout()
plt.savefig('histogramme_rossler')
plt.show()
...
etat_initial = [1, 1, 1]  
points = odeint(rossler, etat_initial, t)
